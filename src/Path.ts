 ...
 */
const isEqual = (p1: Point, p2: Point): boolean =>
  p1.x === p2.x && p1.y === p2.y;
...
const getClosestPoint = (end: Point): Point => {
  if (openList.length === 0) {
    throw new Error("openlist is empty in getClosestPoint");
  }
  openList.sort(
    (a, b) =>
...
const getMoves = (p: Point, end: Point): Point[] | undefined => {
  ...
  for (const e of points) {
    ...
    if (
      board[e.x]?.[e.y] !== undefined &&
      board[e.x][e.y].ball === undefined &&
      !closedList.some((a) => isEqual(a, e))
    ) {
