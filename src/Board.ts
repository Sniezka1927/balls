...
class Board extends Array<Tile[]> {
  ...
  constructor() {
    ...
    this.nextBalls = new Array(config.nextBalls)
      .fill(null)
      .map(() => balls.random());
    this.appendBalls();
  }
  ...
  clearPath(options?: {
    ...
    if (options?.clearOnlyPrev) {
      arr.pop();
    }
    if (options?.clearOnlyBalls) {
      arr = ["prevBall"];
    }
    for (let i = 0; i < this.length; i++) {
      for (let j = 0; j < this[i].length; j++) {
  ...
  handleMove(): number | false {
    ...
    if (ballsToDelete.length === 0 && appendStatus === "full") {
      return false;
    }
    return ballsToDelete.length;
  }
  ...
  private handleStreak(x: number, y: number): { x: number; y: number }[] {
    ...
    const ballsToDelete = streaks
      .filter((a) => a.length >= config.minStreak - 1)
      .flat();
    if (ballsToDelete.length > 0) {
      ballsToDelete.push({ x, y });
    }
